...
- 'Long-short' return = (long_leg_return - short_leg_return) / 2
  (Average so we can compare to FEZ on a similar volatility scale.)
"""
...
def load_benchmark_returns(start: pd.Timestamp, end: pd.Timestamp) -> pd.Series:
    """Load FEZ (iShares Eurozone ETF) as European benchmark."""
    import yfinance as yf
    fez = yf.Ticker("FEZ").history(
        start=start.strftime("%Y-%m-%d"),
        end=end.strftime("%Y-%m-%d"),
        auto_adjust=True,
    )["Close"]
    return fez.pct_change().dropna()
...
def plot_equity_curves(
    ...
    (1 + benchmark).cumprod().plot(ax=ax, label="FEZ (long only)", color="gray", alpha=0.7)
    ...
    # Bottom: drawdowns
    ax = axes[1]
    for ret, label, color in [(gross, "Gross", "green"), (net, "Net", "blue"), (benchmark, "FEZ", "gray")]:
        nav = (1 + ret).cumprod()
...
def main() -> int:
    ...
    # FEZ benchmark (Eurozone ETF)
    print("\nLoading FEZ benchmark...")
    ...
    m_bench = compute_metrics(bench_ret, "FEZ")
    ...
    yearly_bench = yearly_returns(bench_ret).rename(columns={"annual_return": "fez_return"})
    yearly = yearly_net.rename(columns={"annual_return": "ls_net_return"}).join(yearly_bench)
    yearly["excess"] = yearly["ls_net_return"] - yearly["fez_return"]
