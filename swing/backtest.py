...
- Per-fold IC (correlation between predicted and realized) for audit

Conventions:
- Returns are decimal (0.01 = 1%)
- Cost is per leg, one-way: 3 bps default
- 'Long-short' return = (long_leg_return - short_leg_return) / 2
  (Average so we can compare to SPY on a similar volatility scale.)
"""
...
def load_spy_returns(start: pd.Timestamp, end: pd.Timestamp) -> pd.Series:
    """Load SPY returns for the same period for benchmarking."""
...
def plot_equity_curves(
    ...
    (1 + spy).cumprod().plot(ax=ax, label="SPY (long only)", color="gray", alpha=0.7)
    ...
    # Bottom: drawdowns
    ax = axes[1]
    for ret, label, color in [(gross, "Gross", "green"), (net, "Net", "blue"), (spy, "SPY", "gray")]:
        nav = (1 + ret).cumprod()
...
def main() -> int:
    ...
    spy_ret = load_spy_returns(pf.index.min(), pf.index.max())
    # Make tz-naive to align
    spy_ret.index = pd.to_datetime(spy_ret.index).tz_localize(None) if spy_ret.index.tz is not None else pd.to_datetime(spy_ret.index)
    spy_ret = spy_ret.reindex(pf.index, method="ffill").dropna()
    m_spy = compute_metrics(spy_ret, "SPY")
    
    metrics_df = pd.DataFrame([m_gross, m_net, m_spy])
    ...
    yearly_spy = yearly_returns(spy_ret).rename(columns={"annual_return": "spy_return"})
    yearly = yearly_net.rename(columns={"annual_return": "ls_net_return"}).join(yearly_spy)
    yearly["excess"] = yearly["ls_net_return"] - yearly["spy_return"]
